class Weathering::ForecastsController < ApplicationController
  def show
    ...
    logger.debug { "Got coordinates #{geocode_result.coordinates} at postal code #{geocode_result.postal_code}, full address #{geocode_result.address}" }

    forecast =
      Rails.cache.fetch(geocode_result.postal_code, expires_in: 30.minutes) do
